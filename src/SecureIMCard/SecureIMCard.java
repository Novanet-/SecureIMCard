...
public class SecureIMCard extends Applet
{
	...
	private byte[] padSecret(byte[] unpaddedSecret, short secretSize, short keySize)
	{
		...
		for (short i = 0; i < amountToPad)
		{
			unpaddedSecret[secretSize + i] = (byte) 0xFF;
		} return unpaddedSecret;
	}


	private DESKey gen3DESKeyFromSecret(byte[] secret, short secretSize, short keySize)
	{
		DESKey desKey = (DESKey) KeyBuilder.buildKey(KeyBuilder.TYPE_DES_TRANSIENT_RESET, KeyBuilder.LENGTH_DES3_3KEY, false);
		byte[] keyData = padSecret(secret, secretSize, keySize);
		desKey.setKey(keyData, (short) 0);
		return desKey;
	}

	private void initDESCipher(DESKey desKEy)
	{
